...
class SadSplits{
	...
	private static void solve() throws Exception {
		...
		{
			String s = next();
			println(dfs(s, 0, s.length(), 0, 0, false, false) ? "YES" : "NO");
		}
	}

	private static boolean dfs(String s, int index, int n, long a, long b, boolean f1, boolean f2) throws Exception
	{
		if(index == n)
			return f1 && f2 && (a + b) % 2 == 0;

		int x = s.charAt(index) - '0';
		return dfs(s, index + 1, n, a * 10 + x, b, true, f2) || dfs(s, index + 1, n, a, b * 10 + x, f1, true);
	}
}