	...
	private static void solve() throws Exception {
		...
		while(t-- > 0)
		{
			...
			for(int i = 0; i < n; i++)
			{
				...
				List<Long> list = map.getOrDefault(b, new ArrayList<>());
				list.add(a[i]);
				map.put(b, list);
			}
			...
			PriorityQueue<Long> pq = new PriorityQueue<>((a1, b1) -> b1 > a1 ? 1 : -1);

			double total = 0;

			int k = 1;

			for(long x : map.getOrDefault(0, new ArrayList<>()))
				pq.add(x);
			...
			while(pq.size() > 0)
			{
				...
				for(long x : map.getOrDefault(k, new ArrayList<>()))
					pq.add(x);
